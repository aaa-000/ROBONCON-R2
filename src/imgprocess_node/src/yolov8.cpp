...
{
    this->objs.clear();
    this->objs_bucket.clear();
    this->objs_ball.clear();
    this->copy_from_Mat(image, this->size);// 输入的预处理
    this->infer();// 推理
    this->postprocess(this->objs, this->objs_bucket, this->objs_ball);// 对推理结果的后处理
    this->draw_objects(image, this->res, this->objs, this->resultFlag, CLASS_NAMES, COLORS);// 画图展示
}
...
void YOLOv8::postprocess(std::vector<Object>& objs, std::vector<Object>& objs_bucket, std::vector<Object>& objs_ball, float score_thres, float iou_thres, int topk, int num_labels)
{
    objs.clear();
    ...
    for (auto& i : indices) {
        ...
        objs.push_back(obj);
        if (obj.label == 1) {
            objs_bucket.push_back(obj);
        } else {
            objs_ball.push_back(obj);
        }
        cnt += 1;
    }
    ...
    //     }
    // }
    sort(objs.begin(), objs.end(), Compition);
    sort(objs_bucket.begin(), objs_bucket.end(), Compition);
    sort(objs_ball.begin(), objs_ball.end(), Compition);
}
                          ...
                          const std::vector<std::vector<unsigned int>>& COLORS)
{
    ...
    for (auto obj : objs) {
        ...
        char text[256];
        sprintf(text, "%s %.1f%%", CLASS_NAMES[0].c_str(), obj.prob * 100);
